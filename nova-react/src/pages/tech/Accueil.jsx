  ...
  return (
    <>
      <section className="welcome-section">
        <h1>Bonjour <span className="username">{user?.nom || 'Technicien'}</span></h1>
        <p>Bienvenue dans votre espace technicien NOVA.</p>
      </section>
              ...
              {notifications.map(n => {
                ...
                return (
                  <li key={n.id} className={`notif-widget-item${!n.is_read ? ' unread' : ''}`}>
                    ...
                    {n.link && (
                      <button className="notif-voir-btn" onClick={() => navigate(`/tech/${n.link}`)}>
                        Voir
                      </button>
                    )}
                  </li>
